...
alias docker-update=$'docker images --format \'{{.Repository}}:{{.Tag}}\' | sudo -g docker xargs -t -n 1 docker pull'
alias podman-update=$'podman images --filter=dangling=false --format \'{{.Repository}}:{{.Tag}}\' | grep -v -e ^localhost/ | xargs -tr -P4 -n1 podman pull'
alias podman-storage-compare=$'vimdiff <(ls -d ~/.local/share/containers/storage/overlay/* | sort) <(podman image list --format {{.Repository}}:{{.Tag}} | xargs -n1 podman image inspect | jq -r \'.[].GraphDriver.Data | .Upperdir,.LowerDir,.WorkDir | select (. != null)\' | tr : \'\n\' | sort)'
